...
for p in procesos:
    bar = "█" * random.randint(10, 20) + "-" * random.randint(0, 10)
    print(Fore.GREEN + f"{p} 🔋 [{bar}] 100%")
    time.sleep(0.12)

# ─────────────────────────────
# LOADER PRINCIPAL (NORMAL)
# ─────────────────────────────
...
for i in range(25):
    ...
    bar = "█" * random.randint(10, 20)

    print(
        Fore.GREEN +
        f"📡 NODO-{random.randint(1000,9999)} "
        f"UP:{up}KB/s DOWN:{down}KB/s LAT:{lat}ms "
        f"🔋[{bar}] 100%"
    )
    time.sleep(0.05)
