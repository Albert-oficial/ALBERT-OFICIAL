...
process.on('unhandledRejection', (err) => {
  console.log('⚠️ Promesa no manejada:', err?.message || err);
});
process.on('uncaughtException', (err) => {
  console.log('⚠️ Excepción no capturada:', err?.message || err);
});
// --------------------------------------------------
...
const DURACION_PAUSA_MS = 5 * 60 * 1000; // 5 minutos
...
async function iniciarBot() {
  ...
  const sock = makeWASocket({
    ...
    getMessage: async (key) => {
      return almacenMensajes.get(key.id) || undefined;
    }
  });
  ...
  sock.ev.on('messages.upsert', async m => {
    ...
    if (msg.key.fromMe) {
      const remitentePausa = msg.key.remoteJid;
      pausaHasta.set(remitentePausa, Date.now() + DURACION_PAUSA_MS);
      console.log(`⏸️ Pausando IA 5 minutos en este chat (respondiste tú personalmente)`);
      return;
    }
    ...
    if (pausadoHasta && Date.now() < pausadoHasta) {
      const restante = Math.ceil((pausadoHasta - Date.now()) / 1000);
      console.log(`⏸️ IA en pausa en este chat (quedan ${restante}s), no se responde`);
      return;
    }
...
function obtenerInfraestructura() {
  ...
  return {
    ...
    ramTotalSistemaMB: Math.round(os.totalmem() / 1024 / 1024),
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  body {
    background: linear-gradient(160deg, #1c1c1e 0%, #3a3a3d 40%, #0d0d0e 100%);
    color: #e8e8ec;
  ...
  h1 {
    ...
    letter-spacing: 6px;
    font-weight: 800;
    background: linear-gradient(120deg, #cfd4da 0%, #ffffff 25%, #8a8f98 50%, #ffffff 75%, #cfd4da 100%);
