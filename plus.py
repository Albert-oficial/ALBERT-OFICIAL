...
# 🔧 SOLO MEJORA DE VISUAL EN TERMINAL (NO CAMBIA TU BANNER)
...
if cmd == "Cyber/*Root,9911":
    ...
    for i, p in enumerate(procesos):
        ...
        # 🔥 MEJORA: ON/OFF dinámico sin tocar tu estructura
        estado = random.choice(["ON", "ON", "OFF"])
        color = Fore.GREEN if estado == "ON" else Fore.RED

        print(
            color +
            f"[{hora}] {icono} [+] {p} ... {estado}"
        )
    ...
    for i in range(25):
        ...
        # 🔥 MEJORA ON/OFF SIN CAMBIAR TU ESTRUCTURA
